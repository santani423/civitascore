...
export function Header({ onMenuClick }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-20 flex h-16 items-center gap-3 border-b border-border bg-surface/80 px-4 backdrop-blur sm:px-6">
      ...
      {/* Grows to fill all remaining space (uncapped) so the icon group after
          it is always flush against the header's right edge — the search
          box itself is capped via an inner element, not this wrapper, so
          the flex-grow calculation never leaves unclaimed trailing space. */}
      <div className="ml-auto flex flex-1 items-center sm:ml-4">
